    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 max-w-[1600px] mx-auto min-h-[70vh]">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Atendimento</h2>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <select
                        ...
                        className="rounded-xl bg-white/5 border border-white/10 text-sm text-white px-3 py-2.5 min-w-[180px] focus:outline-none focus:border-[#00FF00]/40"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-4 min-h-[560px]">
                <div className="glass-card rounded-2xl border border-white/10 flex flex-col overflow-hidden max-h-[70vh] lg:max-h-none">
                    <div className="p-3 border-b border-white/10">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-gray-600 focus:outline-none"
                            />
                        </div>
                    </div>
                        ...
                        {filtered.map((c) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 border-b border-white/5 hover:bg-white/[0.04] transition-colors ${
                                    selectedId === c.id ? 'bg-[#00FF00]/10 border-l-2 border-l-[#00FF00]' : ''
                                }`}
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        <p className="text-sm font-bold text-white truncate">{c.contactName || c.phoneNumber}</p>
                ...
                <div className="glass-card rounded-2xl border border-white/10 flex flex-col min-h-[480px] max-h-[70vh]">
                    {selected ? (
                        <>
                                        ...
                                        >
                                            <div
                                                className={`max-w-[85%] rounded-2xl px-4 py-2 text-sm ${
                                                    m.fromMe
                                                        ? 'bg-[#00FF00]/20 text-white border border-[#00FF00]/30'
                            ...
                            {!selected.companyId ? (
                                <div className="p-4 border-t border-white/10 text-xs text-amber-200">
                                ...
                                <div className="p-4 border-t border-white/10 flex gap-2">
                                    <textarea
                                        ...
                                        rows={2}
                                        className="flex-1 rounded-xl bg-white/5 border border-white/10 text-sm text-white px-3 py-2 resize-none focus:outline-none focus:border-[#00FF00]/40"
                                        onKeyDown={(e) => {
                                    ...
                                    <button
                                        ...
                                        className="self-end px-4 py-3 rounded-xl bg-[#00FF00] text-black font-black disabled:opacity-40"
                                    >
                                        <Send size={18} />
                                    </button>
                                </div>
