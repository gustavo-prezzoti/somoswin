...
import { Mail, Lock, ArrowRight, Zap, Check, ShieldCheck, Cpu, TrendingUp } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Extrai o nome do e-mail e capitaliza a primeira letra (ex: william@gmail.com -> William)
    const rawName = email.split('@')[0];
    const formattedName = rawName.charAt(0).toUpperCase() + rawName.slice(1).toLowerCase();

    // Persiste o e-mail no objeto do usuário para identificação no dashboard
        ...
        <div className="p-10 md:p-20 space-y-10 flex flex-col justify-center bg-white">
           ...
           <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="flex items-center justify-between px-2 text-[11px] font-black text-gray-400 uppercase tracking-widest">
                 <label className="flex items-center gap-2 cursor-pointer hover:text-emerald-600 transition-colors">
                   <input type="checkbox" className="w-4 h-4 rounded border-gray-200 text-emerald-600 focus:ring-emerald-500" />
                   Manter Conectado
                 </label>
                 <button type="button" className="hover:text-emerald-600 transition-colors">Recuperar Chave</button>
              </div>

              <button 
                type="submit" 
                className="w-full bg-emerald-600 text-white font-black py-6 rounded-[24px] flex items-center justify-center gap-3 hover:bg-emerald-700 transition-all shadow-2xl shadow-emerald-600/40 mt-8 group text-lg tracking-tight"
              >
           ...
           <div className="pt-6 text-center">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                Novo por aqui? <button onClick={() => navigate('/checkout')} className="text-emerald-600 hover:underline ml-1">Ativar Nova Instância</button>
              </p>
           </div>
        </div>
