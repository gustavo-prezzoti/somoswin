...
class AgendaService:
    ...
    def confirmar_agendamento(self, agendamento_id: str) -> Dict:
        """Confirma um agendamento no banco de dados local."""
        try:
            from app.database import get_db
            db = get_db()
            
            if not db.is_connected():
                return {'sucesso': False, 'erro': 'Banco de dados não conectado'}
            ...
            with db.get_session() as session:
                ...
                query = text("""
                    ...
                    WHERE id = :id OR metadata->>'id' = :id_str OR (metadata->>'profissional_id' = :id_str AND metadata->>'telefone' IS NOT NULL)
                """)
                session.execute(query, {'id': int(agendamento_id) if agendamento_id.isdigit() else 0, 'id_str': str(agendamento_id)})
                
            return {'sucesso': True, 'mensagem': 'Agendamento confirmado localmente'}
            
        except Exception as e:
            logger.error(f"Erro ao confirmar agendamento {agendamento_id}: {e}")
            return {'sucesso': False, 'erro': str(e)}

    def cancelar_agendamento_local(self, agendamento_id: str) -> Dict:
        """Cancela um agendamento no banco de dados local (altera status para 'cancelado')."""
        try:
            from app.database import get_db
            db = get_db()
            ...
            with db.get_session() as session:
                ...
                query = text("""
                    ...
                    WHERE id = :id OR metadata->>'id' = :id_str
                """)
                session.execute(query, {'id': int(agendamento_id) if agendamento_id.isdigit() else 0, 'id_str': str(agendamento_id)})
                
            return {'sucesso': True, 'mensagem': 'Agendamento cancelado localmente'}
            
        except Exception as e:
            logger.error(f"Erro ao cancelar agendamento local {agendamento_id}: {e}")
            return {'sucesso': False, 'erro': str(e)}
