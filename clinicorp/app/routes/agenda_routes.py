...
@api_bp.route('/agenda/confirmar', methods=['POST'])
def confirmar_agendamento():
    """Confirma um agendamento localmente.
    ...
    try:
        ...
        if not agendamento_id:
            return jsonify({'erro': 'Campo "id" e obrigatorio'}), 400
            
        resultado = agenda_service.confirmar_agendamento(agendamento_id)
        return jsonify(resultado), 200 if resultado.get('sucesso') else 400
    except Exception as e:
        return jsonify({'erro': str(e)}), 500


@api_bp.route('/agenda/cancelar-local', methods=['POST'])
def cancelar_agendamento_local():
    """Cancela um agendamento localmente.
    ...
    try:
        ...
        resultado = agenda_service.cancelar_agendamento_local(agendamento_id)
        return jsonify(resultado), 200 if resultado.get('sucesso') else 400
    except Exception as e:
        return jsonify({'erro': str(e)}), 500
